...
# Returns comma separated preditions of shooterNames, killedNum, woundedNum and city with shooter names separated by '|'
def predict_mode(sentence, tags, cities):
    output_entities = {}
    for tag in int2tags:
        output_entities[tag] = []

    for j in range(len(sentence)):
        output_entities[int2tags[tags[j]]].append(sentence[j])

    output_pred_line = ""
    for shooterName in output_entities["shooterName"]:
            ...
            for permutation in itertools.permutations(output_entities[tag],2):
                if permutation[0] in cities:
                    if "" in cities[permutation[0]]:
                        possible_city_combos.append(permutation[0])
                    if permutation[1] in cities[permutation[0]]:
                        possible_city_combos.append(permutation[0] + " " + permutation[1])
            mode = get_mode(possible_city_combos)
            if mode == "":
                possible_cities = []
                for i in range(len(sentence)):
                    ...
                    if word1 in cities:
                        if "" in cities[word1]:
                            possible_cities.append(word1)
                        if i+1 < len(sentence):
                            word2 = sentence[i+1]
                            if word2 in cities[word1]:
                                possible_cities.append(word1 + " " + word2)

                #print possible_cities
                #print get_mode(possible_cities)
                output_pred_line += get_mode(possible_cities)
            else:
                output_pred_line += mode
        elif tag not in ["TAG", "shooterName"]:
            output_pred_line += ","
            mode = get_mode(output_entities[tag])
            if mode == "":
